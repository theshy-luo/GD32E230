...
| **`Utilities/gd32e230c_eval.c/h`** | **硬件接口** | GPIO 极性控制、ADC/DMA 初始化、**各通道独立阈值配置表**。 |
...
- **阈值匹配**：系统根据当前通道索引，从 `adc_chan_thresholds[8]` 中提取对应的 `min` 和 `max` 范围。
- **OK 计数**：若均值在设定的范围内，`stable_ok_cnt++`。达到 **10** 次，`is_ok = 1`。
- **Error 计数**：若均值溢出范围，`stable_err_cnt++`。达到 **5** 次，`is_ok = 0`。
...
- **开发者只需要关注业务语义**：调用 `gd_eval_power_en_set(idx, 1)` 即代表“使能/开启”，底层会自动映射到正确的高低电平。

### 独立阈值配置
系统允许通过以下两种方式配置每一路 ADC 的判定范围：
1. **静态配置**：直接修改 `gd32e230c_eval.c` 中的 `adc_chan_thresholds` 初始数组。
2. **动态配置**：使用 `gd_eval_adc_set_threshold(index, min, max)` 接口。
   - 配置是**即时生效**的：下一次中断处理（~2.3ms后）就会开始使用新阈值。
...
如果您在实测中发现系统卡在某个启动步骤：
1. **验证阈值**：确认您为该通道配置的 `min/max` 是否匹配分压后的实际物理电压。
2. **查看均值**：通过仿真器查看 `adc_chan_states` 数组中对应通道的 `avg` 值。
